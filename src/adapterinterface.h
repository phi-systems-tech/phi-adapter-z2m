...
public:
    ...
    void stopAdapter() { stop(); }
    void notifyConfigUpdated() { adapterConfigUpdated(); }
    void triggerFullSync() { requestFullSync(); }
    void invokeChannelUpdate(const QString &deviceExternalId,
                             const QString &channelExternalId,
                             const QVariant &value,
                             CmdId cmdId)
    {
        updateChannelState(deviceExternalId, channelExternalId, value, cmdId);
    }
    void invokeAction(const QString &actionId, const QJsonObject &params, CmdId cmdId)
    {
        invokeAdapterAction(actionId, params, cmdId);
    }
    void invokeNameUpdate(const QString &deviceId, const QString &name, CmdId cmdId)
    {
        updateDeviceName(deviceId, name, cmdId);
    }
    void invokeSceneAction(const QString &sceneExternalId,
                           const QString &groupExternalId,
                           const QString &action,
                           CmdId cmdId)
    {
        invokeScene(sceneExternalId, groupExternalId, action, cmdId);
    }
    void invokeEffect(const QString &deviceExternalId,
                      phicore::adapter::DeviceEffect effect,
                      const QString &effectId,
                      const QJsonObject &params,
                      CmdId cmdId)
    {
        invokeDeviceEffect(deviceExternalId, effect, effectId, params, cmdId);
    }
    void setStaticConfig(const QJsonObject &config) { updateStaticConfig(config); }
