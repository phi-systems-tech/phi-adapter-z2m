...
/// A single click is reported at once. The 1.3 seconds it used to wait, in
/// case a second click followed, made the most common thing a button does the
/// slowest thing the adapter did.
void testAClickIsNotHeldBack()
{
    ButtonPresses presses;
    const std::string key = "0x1:button1";

    auto out = presses.onEvent(key, Code::InitialPress, 1000);
    PHI_CHECK(out.size() == 1 && out[0] == Code::InitialPress);
    out = presses.onEvent(key, Code::ShortPressRelease, 1100);
    PHI_CHECK_MSG(out.size() == 1 && out[0] == Code::ShortPressRelease,
                  "a single click was not reported at once");

    // A second click inside the window is reported as a double, in addition.
    presses.onEvent(key, Code::InitialPress, 1300);
    out = presses.onEvent(key, Code::ShortPressRelease, 1400);
    PHI_CHECK(out.size() == 2 && out[0] == Code::ShortPressRelease && out[1] == Code::DoublePress);
    presses.onEvent(key, Code::InitialPress, 1600);
    out = presses.onEvent(key, Code::ShortPressRelease, 1700);
    PHI_CHECK(out.size() == 2 && out[1] == Code::TriplePress);

    // A click well after the window starts over.
    presses.onEvent(key, Code::InitialPress, 5000);
    out = presses.onEvent(key, Code::ShortPressRelease, 5100);
    PHI_CHECK(out.size() == 1 && out[0] == Code::ShortPressRelease);

    // Holding: the first is a long press, the ones that follow are repeats,
    // and a release ends the run.
    out = presses.onEvent(key, Code::LongPress, 8000);
    PHI_CHECK(out.size() == 1 && out[0] == Code::LongPress);
    out = presses.onEvent(key, Code::LongPress, 8400);
    PHI_CHECK(out.size() == 1 && out[0] == Code::Repeat);
    out = presses.onEvent(key, Code::LongPress, 8700);
    PHI_CHECK(out.size() == 1 && out[0] == Code::Repeat);
    out = presses.onEvent(key, Code::LongPressRelease, 8900);
    PHI_CHECK(out.size() == 1 && out[0] == Code::LongPressRelease);
    out = presses.onEvent(key, Code::LongPress, 9000);
    PHI_CHECK(out.size() == 1 && out[0] == Code::LongPress);
    ...
    // A device that counts for itself is believed.
    out = presses.onEvent(key, Code::DoublePress, 9500);
    PHI_CHECK(out.size() == 1 && out[0] == Code::DoublePress);
}
